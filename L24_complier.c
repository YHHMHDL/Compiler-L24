...
#define stacksize 1000 /* 运行时数据栈元素最多为1000个 */
#define maxvar 50    /*最大的变量数目,由于隐式变量，需要提前指定*/
/* 符号 */
enum symbol {
...
void listall();
void vardeclaration(int* ptx, int lev, int* pdx);//变量声明
void constdeclaration(int* ptx, int lev, int* pdx);//整型变量声明
void implicitconstdeclaration(int* ptx, int lev, int* pdx);//隐式变量声明
  ...
  //递归下降语法分析
    if(sym==mainsym){
      ...
      if(sym==lbrace){
        ...
        cx0 = cx;
        gen(ini, 0, maxvar);	            /* 生成指令，此指令执行时在数据栈中为被调用的过程开辟dx个单元的数据区 */

        bool nxtlev[symnum];
        /* stmt_list后继符号为右大括号*/
        memcpy(nxtlev, fsys, sizeof(bool) * symnum);
        nxtlev[rbrace] = true;
        stmt_list(0, &tx,&dx,nxtlev); /*处理stmt_list*/
...
void stmt_list(int lev , int *ptx,int *pdx,bool* fsys)
{
  ...
  if (sym==semicolon)
  {
    getsym();//取semicolon后面一个字符，判断是否结束st_m
  }
  else {
    error(88);//语句后跟符号不是‘；’
    }
...
void statement(bool* fsys, int* ptx, int lev,int* pdx) {
    ...
    if (sym == ident) {    /* 准备按照赋值语句处理 */
        j = position(id,*ptx);/* 查找标识符在符号表中的位置 */
        if (j == 0) {
            //error(11);   /* 标识符未声明,则声明新变量*/
            ...
            if (sym == rparen) {
                ...
                if (sym == thensym) {
                    ...
                    if (sym == lbrace) {
                        ...
                        memcpy(nxtlev, fsys, sizeof(bool) * symnum);
                        nxtlev[rbrace] = true;
                        //statement(nxtlev, ptx,lev,pdx);    /* 处理then后的语句 */
